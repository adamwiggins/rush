...
describe Rush::Connection::Local do
  ...
  it "receive -> processes" do
    @con.should_receive(:processes).with(no_args).and_return([ { :pid => 1 } ])
  ...
  it "create_dir creates a directory" do
    ...
    File.directory?(fname).should eq(true)
  end

  it "rename to rename entries within a dir" do
    system "touch #{@sandbox_dir}/a"
    @con.rename(@sandbox_dir, 'a', 'b')
    File.exists?("#{@sandbox_dir}/a").should eq(false)
    File.exists?("#{@sandbox_dir}/b").should eq(true)
  end
  ...
  it "copy to copy an entry to another dir on the same box" do
    ...
    File.exists?("#{@sandbox_dir}/a").should eq(true)
    File.exists?("#{@sandbox_dir}/subdir/a").should eq(true)
  end
  ...
  it "write_archive to turn a byte stream into a dir" do
    ...
    File.directory?("#{@sandbox_dir}/dst/a").should eq(true)
    File.exists?("#{@sandbox_dir}/dst/a/b").should eq(true)
  end
  ...
  it "write_archive works for paths with spaces" do
    ...
    File.directory?("#{@sandbox_dir}/with space/a").should eq(true)
    File.exists?("#{@sandbox_dir}/with space/a/b").should eq(true)
  end
...
EOPS
  ...
  it "the current process should be alive" do
    @con.process_alive(Process.pid).should eq(true)
  end

  it "a made-up process should not be alive" do
    @con.process_alive(99999).should eq(false)
  end
