...
describe Rush::SshTunnel do
  before do
    ...
    allow(@tunnel).to receive(:config).and_return(mock_config_start)
    allow(@tunnel).to receive(:display)
  end
  ...
  it "ensure_tunnel sets everything up for the tunnel when one does not already exist" do
    expect(@tunnel).to receive(:push_credentials)
    expect(@tunnel).to receive(:launch_rushd)
    expect(@tunnel).to receive(:establish_tunnel)
    @tunnel.ensure_tunnel
  end

  it "ensure_tunnel uses the existing port as long as the tunnel is still alive" do
    expect(@tunnel).to receive(:tunnel_alive?).and_return(true)
    @tunnel.instance_eval("@port = 2345")
    @tunnel.ensure_tunnel
    expect(@tunnel.port).to eq(2345)
  end
  ...
  it "tunnel_alive? checks whether a tunnel is still up" do
    ...
    expect(@tunnel.tunnel_alive?).to eq(true)
  end
